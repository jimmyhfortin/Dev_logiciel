...
-- ------------------------------------------------
-- requetes avancer 
CREATE DATABASE IF NOT EXISTS Laboratoire06;
USE Laboratoire06;
-- --------------------------------------------------------
--
...
-- Solution requêtes laboratoire 06
...
insert into Clients(Nom,Prénom,statut) values("Godin","Gérald",1);
insert into Clients(Nom,Prénom,statut) values("Gosselin","Nancy",0),("Tijani","Ahmed",0),("Roy","Mélanie",1);
insert into Clients(Nom,Prénom,statut) values("Roy","Marc-Antoine",1);
-- Table Produits
insert into Produits values(10,"Croustilles ondulées nature");
insert into Produits values(20,"Barres de céréales Great Value");
insert into Produits values(30,"Gâteaux Twinkiesᴹᴰ de Hostess");
...
FROM
    Achats;*/
-- INNER JOIN : jointure interne : correspondance entre les lignes des deux tables
-- insert into Clients(IDClient,Nom,Prénom,statut) values(1,"Errahmouni","Hind",1);
select C.IDClient,Nom,Prénom,Qte_Achetée from Clients C inner join Achats  A on C.IDClient = A.IDClient; -- 1,3,4,5,6
-- OUTER JOIN: LEFT/RIGHT
select distinct C.IDClient,Nom,Prénom,Qte_Achetée from Clients C left outer join Achats  A on C.IDClient = A.IDClient; -- 1,2,3,4,5,6,7
select distinct C.IDClient,Nom,Prénom,Qte_Achetée from Clients C right outer join Achats  A on C.IDClient = A.IDClient;
-- CROSS JOIN
select * from Clients,Achats;
select * from Clients cross join Achats;
-- NATURAL JOIN
select C.IDClient,Nom,Prénom,Qte_Achetée from Achats natural join Clients C;
