...
public class Line
{
    ...
    {
        int lineY = Math.Abs(StartPoint.Y - EndPoint.Y);
        //line = (StartPoint, Math.Abs(EndPoint.Y-StartPoint.Y), DrawColor);
        // ligne verticale, il faut éviter une division par 0
        if (StartPoint.X == EndPoint.X)
        {
            VLine line = new VLine(StartPoint, height:lineY, DrawColor);
            //line = (StartPoint.X, Math.Min(StartPoint.Y, EndPoint.Y), Math.Abs(EndPoint.Y - StartPoint.Y) + 1, DrawColor);
            line.Draw(canvas);
            return;
        }
